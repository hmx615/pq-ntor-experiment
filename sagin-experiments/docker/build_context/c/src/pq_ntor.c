...
/**
 * 辅助函数：派生会话密钥
 * (K_auth || K_enc) = HKDF(K_kem, transcript_hash, "pq-ntor-keys")
 */
static int derive_keys(uint8_t *k_auth, uint8_t *k_enc,
    ...
    // 对 transcript 进行哈希作为 salt
    if (sha256_hash(transcript_hash, transcript, transcript_len) != CRYPTO_SUCCESS) {
        return PQ_NTOR_ERROR;
    }

    // HKDF: 使用 transcript_hash 作为 salt，固定字符串作为 info
    const char *info = PQ_NTOR_INFO_STRING;
