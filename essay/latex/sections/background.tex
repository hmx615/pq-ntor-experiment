...
% Final timeline endpoints
\coordinate (a_end) at ($(box4.south) + (0,-0.15)$);
\coordinate (b_end) at ($(box5.south) + (0,-0.15)$);
...
Table~\ref{tab:key-comparison} compares key sizes across different schemes. While Kyber's keys are significantly larger than X25519's 32-byte keys, they remain manageable in modern network protocols. Following cryptographic best practices~\cite{ietf-hybrid-kem}, we employ a \emph{hybrid} approach combining Kyber-512 with X25519. This ensures security against both classical attacks (via X25519) and future quantum attacks (via Kyber), providing defense-in-depth even if one primitive is compromised.
% 【中文翻译】表~\ref{tab:key-comparison}比较了不同密钥交换机制的密钥大小。虽然Kyber的密钥大小显著大于X25519的32字节密钥，但在现代网络协议中仍然是可管理的。遵循密码学最佳实践，我们采用混合方法，将Kyber-512与X25519结合。这确保了对经典攻击（通过X25519）和未来量子攻击（通过Kyber）的双重防护，即使一个基元被攻破也能提供纵深防御。
